...
export default function MasmedPage() {
  const { loading, rounds, heads, participants } = useGame();
  ...
  if (!closed.length) return <p className="text-sm text-neutral-500">No closed session yet.</p>;
      ...
      <div className="bg-white text-black rounded-lg p-6 border">
        ...
        {E && (
          <div className="border-2 border-blue-700 bg-blue-50 rounded-lg p-3 mt-4">
            ...
            {E.announce && (
              <div className="mt-1">
                A national election will be held <b>next session</b> between <b>POP</b> ({headOf("POP")}) and{" "}
                <b>SOP</b> ({headOf("SOP")}). The winning party gains primary control of the treasury. Both party
            ...
            {E.winner && (
              <div className="mt-1">
                ...
                <div className="mt-1">
                  ...
                  {E.treasury.recips.map((g) => `${g} head (${headOf(g)}): $${fmt(E.treasury.share)}`).join(" · ")}.
                </div>
              </div>
            )}
